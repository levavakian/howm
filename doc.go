/*
howm is a hybrid tiling manager aiming to replicate the ergonomics of terminator and tmux.
...
cmd/howmbright/ - a utility for changing backlight brightness without root privileges
...
compile.sh - go gets/builds howm in container
clean.sh - gets rid of container created by dev.sh
exec.sh - execs into container created by dev.sh
install.sh - (requires root) builds howm and installs files to the system (only tested on ubuntu18.04)
test.sh - (requires xephyr to be installed and dev.sh to have been run) opens a nested x server with howm loaded for trying out
*/
package main
